...
3. In **AI Provider**, select **“Fine-tuned (SpeechGradebook) – Your trained Mistral 7B”**.
...
| Issue | What to try |
|-------|-------------|
| `FileNotFoundError: train.jsonl` | Run from `SpeechGradebook/llm_training` or pass full path: `--train_file /path/to/train.jsonl`. |
...
| App says “Fine-tuned server error” | Check server is running (`curl http://localhost:8000/health`), URL has no trailing slash, and CORS is allowed. |
...
- [ ] In app: provider = Fine-tuned, URL = `http://localhost:8000`, run evaluation
