...
| **B** | Export → convert to JSONL → train LoRA → serve | Fine-tuned model loaded; app uses it for evaluations |
...
2. **Start again** from the repo root:
   ...
   Or: `bash "/path/to/SpeechGradebook Repo/run_local.sh"`
...
3. Set **Evaluation provider** to **Fine-tuned (SpeechGradebook)** and ensure the evaluation server URL is **http://localhost:8000/api** (or your server URL).
...
| 9 | In the app, run an evaluation with **Fine-tuned (SpeechGradebook)**. |
