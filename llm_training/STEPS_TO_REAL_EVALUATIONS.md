...
From the current state (app + server running, no trained model → 503 “Model not loaded”) to real evaluations using your SpeechGradebook Text Model (Mistral) (fine-tuned Mistral 7B).
...
4. **Restart the server:**  
   Stop `./run_local.sh` (Ctrl+C), then run it again. The app loads the adapter from `llm_training/mistral7b-speech-lora` and evaluations will use the SpeechGradebook Text Model (Mistral).
