...
- **evaluateSpeech()** routes by provider (`finetuned`, `gpt4o`, `gemini`, `claude`, `demo`). The evaluation flow supports `apiProvider = 'finetuned'` (SpeechGradebook Text Model (Mistral)) and `apiProvider = 'qwen'` (SpeechGradebook Text + Video Model (Qwen)); the Mistral server URL is used for the text model., so the API providers are not reachable from the current evaluation UI.
...
**Idea:** Use an **existing LLM via API** (Gemini, OpenAI, or Claude) so you get real evaluations immediately. You correct in the app; the app stores **transcript**, **evaluation_data** (sections, corrections), and **video_notes** (when using Gemini or SpeechGradebook Text Model (Mistral) + video) when you save. Each saved evaluation becomes a training example.
...
- **Step 2 — File and rubric:**
  ...
  - In **Evaluation provider**, choose:
    ...
    - **SpeechGradebook Text Model (Mistral)** — for your own model (requires a trained model and, for video, a Gemini key for video notes). **SpeechGradebook Text + Video Model (Qwen)** — evaluates video directly (analyzes both video and text).
...
- On save, the app stores:
  ...
  - **evaluation_data**: `sections` (final scores/comments), `video_notes` (if Gemini or SpeechGradebook Text Model (Mistral) + video), `corrections`, `overallComments`, etc.,
...
- If you used Gemini (or SpeechGradebook Text Model (Mistral) + video with a Gemini key), **evaluation_data.video_notes** will be set; the export script and training pipeline will include it so the model learns from video content too.
...
3. **Ensure transcript is stored**  
   - When you save after a SpeechGradebook Text Model (Mistral) evaluation, the app already saves `evaluation.transcript`. For an API-based flow, the app must set `evaluation.transcript` from the transcription step before save so it’s in the DB.
...
1. **API-based evaluation in the app**  
   - One evaluation path that calls OpenAI or Claude with rubric + transcript, parses response to `sections`, and displays like SpeechGradebook Text Model (Mistral).  
...
3. **Use existing tooling**  
   ...
   - Current app + server for SpeechGradebook Text Model (Mistral)
...
| 5 | After 30–100+ corrected evals, train; then switch to your SpeechGradebook Text Model (Mistral) server for future evaluations. |
...
1. **At inference:** When you use the **SpeechGradebook Text Model (Mistral)** provider with a **video** file and have a **Gemini API key** set in Settings, the app calls Gemini to get a short “video notes” paragraph (body movement, eye contact, gestures, slides, etc.), then sends **transcript + video_notes** to your SpeechGradebook Text Model (Mistral) server. The server builds the prompt as `Transcript: … Video notes: …` + Rubric and runs the model.
2. **Training data:** Export from Supabase includes `video_notes` when present. `export_to_jsonl.js` appends `Video notes: …` to the user prompt for those examples. After training on corrected Gemini (or SpeechGradebook Text Model (Mistral) + video) evaluations that include video_notes, the model learns to use visual delivery in its scores and feedback.

**Summary:** Use **Gemini** for initial video evaluations (and correct/save); video_notes are stored automatically. For **SpeechGradebook Text Model (Mistral)** with video, set a Gemini API key so the app can fetch video notes and send them to your model. Export → train as usual; the pipeline includes video_notes in training when available.
